...
//수를 입력받는 메서드
int wbglobal::getnum(const char* title)
{
    int num;
    char buf[255 + 1];

    cout << title << " : ";
    cin.getline(buf, 255); //버퍼에 입력받음//gets함수와 역할 동일하다
...
//문자열을 입력받는 메서드
const char* wbglobal::getstr(const char* title)
{
    char buf[255 + 1];
...
//기능 키(F1..)를 입력받는 메서드
keydata wbglobal::getkey()       //#include <conio.h>
{
    int key = _getch();
