...
namespace _1012Net
{
    class wbServer
    {
        ...
        /// <summary>
        /// 대기 소켓 create , bind, listen, accept(thread 기반)
        /// </summary>
        /// <param name="port">서버 소켓 생성시 사용될 포트 번호</param>
        ...
        #region start server에서 내부적으로 호출하는  함수들(시작 함수)
        ...
        private Socket Accept()
        {
            ...
            Console.WriteLine("서버 주소 - {0}:{1}", "  127.0.0.1", curip.Port);//>
        ...
        private void WorkThread(object obj)
        {
            ...
            while(true)
            {
                ...
                if(ReceiveData(clientsock, ref recvbyte)==false)
                    break;
                //데이터 처리
               byte[] sendbyte =  Control.Singleton.RecvData(recvbyte);
                //1대1 전송 (unicast)
                SendData(clientsock, sendbyte);
                //1대 다 전송=====================================(multicast)
        ...
        private bool ReceiveData(Socket sock,ref byte[] data)
        {
            try
            {
                ...
                recv_data = sock.Receive(data_size, 0, 256, SocketFlags.None);
