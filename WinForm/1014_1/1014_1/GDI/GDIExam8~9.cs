...
namespace _1014_1
{
    public partial class GDIExam8 : Form
    {
        ...
        protected override void OnPaint(PaintEventArgs pea)
        {
            ...
            g.DrawArc(pen, 10, 10, 100, 100, 0, 90);


            Pen pen1 = new Pen(Color.Red, 2);// 빨간색2의 두께를 갖는 선으로
            g.DrawPolygon(pen1, point1);// 5개 점을 경유하는 다각형 그림
            pen1 = new Pen(Color.Blue, 3);// 파랑색 3의 두께를 갖는 선으로
            g.DrawPolygon(pen1, point2); // 3개의 점을 경유하는 다각형
            pen1 = new Pen(Color.Black, 1);// 두께 1의 검은색 선으로
            for (int i = 0; i < 200; i += 20) // 9개의 타원을 그림
            {
                g.DrawEllipse(pen1, 70, 130, i, i + 50);
            }
        }
    }
