...
namespace Test
{
    public partial class Form1 : Form
    {
        ...
        private void PrintStudent()
        {
            ...
            stulist = StuManager.Singleton.PrintAll();
            foreach (KeyValuePair<int, Student> stu in stulist)
            {
                string subject = transSub(stu.Value.stype);
                listView1.Items.Add(new ListViewItem(new string[] {
                    stu.Value.number.ToString(),
                    stu.Value.name,
                    subject,
                    stu.Value.Grade.ToString()
                     }));
            }
        ...
        private void listView1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count != 0)
            {
                ...
                //textBox2.Text = listView1.Items[SelectRow].SubItems[1].Text;
                //int stype = transSubtoInt(listView1.Items[SelectRow].SubItems[2].Text);
                //comboBox1.SelectedIndex = stype;
                //comboBox2.SelectedIndex = int.Parse(listView1.Items[SelectRow].SubItems[3].Text)-1;
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                Student stu = StuManager.Singleton.NumberToStudent(int.Parse(textBox1.Text));
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                StuManager.Singleton.RemoveStudent(int.Parse(textBox1.Text));
                PrintStudent();
                TextClear();
            }
            catch
        ...
        {
            StuManager.Singleton.EditStudent(int.Parse(textBox1.Text),(SubjectType)(comboBox1.SelectedIndex+1),(comboBox2.SelectedIndex+1));
            PrintStudent();
            TextClear();
        }
        #endregion
